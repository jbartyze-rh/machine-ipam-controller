...
LDFLAGS="${LDFLAGS} -X github.com/jbartyze-rh/mapi-static-ip-controller/pkg/version.Raw=${GIT_TAG} -X github.com/jbartyze-rh/mapi-static-ip-controller/pkg/version.Commit=${GIT_COMMIT}"
TAGS="${TAGS:-}"
...
go build "${GOFLAGS}" -ldflags "${LDFLAGS}" -tags "${TAGS}" -o "${OUTPUT}" ./cmd/mapi-static-ip-controller
